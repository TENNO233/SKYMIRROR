...
def render_case(case: dict[str, Any], index: int) -> str:
    ...
    ex_str = (
        ", ".join(str(expert) for expert in active_experts)
        if active_experts
        else "_(no expert activation)_"
    )
...
def render_empty_day_report(target_date, case_label: str) -> str:
    case_intro = {
        ...
        "B": (
            "RunRecord log file exists but contains no records — the runtime started "
            "but processed nothing."
        ),
        "C": (
            "RunRecord log contains records but no alerted runs — this may indicate "
            "conservative routing, stale rules, or a genuinely quiet day."
        ),
