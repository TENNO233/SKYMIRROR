...
_SURVEILLANCE_SUMMARY_STANDARD = (
    ...
    "- Sentence 1 must state the observable traffic operating condition using direct "
    "facts and routing-useful terms.\n"
    "- Sentence 2 must state the government relevance: enforcement concern, safety "
    "risk, traffic-flow issue, roadway/environment hazard, or that no immediate "
    "action is indicated.\n"
...
def _build_validator_prompt(candidate_report: VlmSceneReport) -> str:
    return (
        ...
        "- If the candidate report overstates, speculates, or conflicts with the image, "
        "correct it conservatively.\n"
        "- normalized_description must be concise, factual, suitable for downstream "
        "routing, and written as an operations-facing surveillance brief.\n"
        ...
        "- discarded_claims should list the candidate claims you removed or softened "
        "because they were unsupported, too strong, or inaccurate.\n\n"
...
def _collapse_to_single_sentence(text: str) -> str:
    ...
    parts = [
        segment.strip(" ,;:")
        for segment in _SENTENCE_BREAK_RE.split(cleaned)
        if segment.strip(" ,;:")
    ]
    if not parts:
        return ""
...
def _build_scene_assessment(report: ValidatedSceneReport) -> str:
    ...
    if signals.queueing and not _contains_any(
        flattened, ("queue", "queueing", "congestion", "traffic jam", "gridlock")
    ):
        addenda.append("queueing is visible")
    if signals.blocked_lanes > 0 and not _contains_any(
        flattened, ("blocked lane", "lane obstruction", "occupying lane")
    ):
    ...
    if signals.water_present and not _contains_any(
        flattened, ("standing water", "flood", "flooding", "waterlogged")
    ):
        addenda.append("standing water is visible")
    if signals.collision_cue and not _contains_any(
        flattened, ("collision", "accident", "crash", "rollover", "overturned")
    ):
        addenda.append("collision indicators are visible")
    if signals.wrong_way_cue and not _contains_any(
        flattened, ("wrong way", "against traffic", "no entry")
    ):
        addenda.append("possible wrong-way movement is visible")
    if signals.low_visibility and not _contains_any(
        flattened, ("low visibility", "poor visibility", "fog", "smoke", "glare", "poor lighting")
    ):
...
def _build_government_relevance(report: ValidatedSceneReport) -> str:
    ...
    if (
        signals.collision_cue
        or signals.wrong_way_cue
        or signals.dangerous_crossing_cue
        or signals.conflict_risk_cue
    ):
        add("immediate safety review is warranted")
    elif _contains_any(flattened, _SAFETY_KEYWORDS):
        add("safety review is warranted")

    if signals.blocked_lanes > 0 or _contains_any(
        flattened, ("blocked lane", "lane obstruction", "occupying lane")
    ):
        add("traffic management attention is warranted because lane capacity appears reduced")

    if (
        signals.water_present
        or signals.construction_present
        or signals.obstacle_present
        or signals.low_visibility
        or signals.lighting_abnormal
    ):
    ...
    if not concerns:
        return (
            "no immediate enforcement, safety, or maintenance action is indicated; "
...
def validator_agent_node(state: SkymirrorState) -> dict[str, Any]:
    ...
    if not candidate_payload:
        fallback_text = str(state.get("vlm_text", "")).strip()
        if not fallback_text:
            raise ValueError("validator_agent_node requires state['vlm_output'].")
        return {
            "validated_scene": {},
            "validated_text": fallback_text,
            "validated_signals": {},
            "metadata": {
                "validator": {
                    "provider": "compat_passthrough",
                    "review_mode": "legacy_vlm_text_passthrough",
                    "summary_standard": "compatibility_passthrough",
                    "input_sources": ["vlm_text"],
